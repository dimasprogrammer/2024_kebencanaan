$(document).on('submit', '#formEntry-tersalurkan', function(e) {
e.preventDefault();
...
$('#save-tersalurkan').html('<i class="spinner-grow spinner-grow-sm mr-2" role="status" aria-hidden="true"></i> DIPROSES...');
$('#save-tersalurkan').addClass('disabled');
$.ajax({
url: url,
type: "POST",
data: data,
...
if (result.value) {
$('#errEntry-tersalurkan').html(msg.error('Silahkan dilengkapi data pada form inputan dibawah'));
$('#frmEntry').waitMe('hide');
}
})
}
}).fail(function() {
$('#errEntry-tersalurkan').html(msg.error('Harap periksa kembali data yang diinputkan'));
swalAlert.fire({
title: 'Terjadi Kesalahan',
...
$('#save-tersalurkan').html('<i class="fas fa-check"></i> Simpan Data ');
$('#save-tersalurkan').removeClass('disabled');
});
});

function getDataTersalurkan(){
let wil_village = $('#wil_village').val();
let token_bencana_detail = $('input[name="token_bencana_detail"]').val();
let url = siteUri + '/review/getDataTersalurkan';
let data = {
...
if(data.status == 'RC200'){
data.data.tersalurkan.forEach(function(item) {
$('#jumlah_bantuan-' + item.id_jenis_bantuan).val(item.jumlah_bantuan);
});
data.data.sumber.forEach(function(item) {
$('#jumlah_sumber-' + item.id_jenis_bantuan).val(item.jumlah_sumber);
});
...
$('#formEntry-tersalurkan input').val(0);
