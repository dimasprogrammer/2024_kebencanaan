...
class Model_users extends CI_Model
{
	...
	public function insertDataUsers()
	{
		...
		$id_instansi   	 = escape($this->input->post('id_instansi', TRUE));
	...
	{
		// $dataopd = file_get_contents('http://simpeg.bkd.sumbarprov.go.id/webapi/instansi/unit-kerja/utama/token/XBnKaywRCrj05m-XXX-v6DXuZ3FFkUgiw45');
		$dataopd = file_get_contents('opd.json');
		$konvert = json_decode($dataopd, true);
		$dataKonvert = $konvert['result'];
		// print_r($dataKonvert);
		// die;
		$dd = '<option value="">Pilih Data Instansi</option>';
		array_sort_by_column($dataKonvert, 'opdName', SORT_ASC);
		if (count($dataKonvert) > 0) {
			foreach ($dataKonvert as $row) {
