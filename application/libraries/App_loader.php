...
class App_loader
{
    ...
    public function current_instansi_name()
    {
        $group_name = $this->CI->session->userdata('group_name');
        return $group_name;
    }
    ...
    {
        if ($this->CI->session->userdata('nick_level') == 'OPD') {
            return TRUE;
        }
        return FALSE;
    }
        ...
        if (isset($menu['items'][$parent])) {
            ...
            foreach ($menu['items'][$parent] as $key) {
                ...
                if ($menu['parents'][$parent][$key]['is_parent'] == 'N') {
                    if ($icon != '')
                        $html .= '<li class="' . (($idMenu == 1) ? 'active' : '') . '"><a href="' . $target . '" class="collapsible-header waves-effect arrow"><i class="' . $icon . '"></i> <span>' . $title . '</span></a></li>';
                    else
                        $html .= '<li><a href="' . $target . '" class="collapsible-header waves-effect arrow"><i class="far fa-circle"></i><span>' . $title . '</span></a></li>';
                } else {
                    if ($icon != '')
                        $html .= '<li>
                                    <a href="' . $target . '" class="collapsible-header waves-effect arrow">
                                        <i class="' . $icon . '"></i>
                                        <span>' . $title . '</span>
                        ...
                        $html .= '<li>
                                    <a href="' . $target . '" class="collapsible-header waves-effect arrow" data-toggle="collapse" aria-expanded="false">
                                        <span>' . $title . '</span>
                                        <i class="fas fa-angle-down rotate-icon"></i>
                                    </a>';
